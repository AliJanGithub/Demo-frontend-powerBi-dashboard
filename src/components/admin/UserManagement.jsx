...
// console.log("userssssssssss acess ssssssssss",users)
...
// export function UserManagement() {
//   const { showToast } = useToast();
//   const { invitations, sendInvitation, deleteInvitation, dashboards, reports } = useData();
//   const { addUser, error, getUsers, inviteUser, users, deleteUser ,loadings} = useUserManagement();
//   const [isInviteDialogOpen, setIsInviteDialogOpen] = useState(false);
//   const [inviteEmail, setInviteEmail] = useState('');
//   const [name, setName] = useState("");
//   const [isLoading, setIsLoading] = useState(false);
...
// };
// const invitationCheck = () => {
//   const res = users && users.length > 0 ? users.filter((obj) => obj.isActive !== true) : [];
//   setPendingNumbers(res);
// };
...
//   const getStatusBadge = (status) => {
//     if (status === true) {
//       return <CustomBadge variant="active">Active</CustomBadge>;
//     } else {
//       return <CustomBadge variant="inactive">Inactive</CustomBadge>;
//     }
//   };
...
//     if (status === true) {
//       return (
//         <CustomBadge variant="pending" className="flex items-center">
...
//           <p className="text-gray-600 mt-1">Manage users and their dashboard access</p>
//         </div>
...
//                       required
//                       disabled={isLoading}
//                       className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-50 disabled:cursor-not-allowed"
//                     />
//                     {emailError && (
//                       <p className="text-sm text-red-600 mt-2">{emailError}</p>
//                     )}
...
//                     }}
//                     disabled={isLoading}
//                     className="px-4 py-2.5 border border-gray-300 rounded-lg hover:bg-gray-50 text-gray-700 disabled:opacity-50"
//                   >
//                     Cancel
//                   </button>
//                   <button
...
export function UserManagement() {
  const { showToast } = useToast();
  const { getUsers, inviteUser, users, deleteUser } = useUserManagement();
  const [isInviteDialogOpen, setIsInviteDialogOpen] = useState(false);
  const [inviteEmail, setInviteEmail] = useState('');
  const [name, setName] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  ...
  const LoadingAnimation = () => (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <div className="h-8 bg-gradient-to-r from-gray-200 to-gray-100 rounded-lg w-64 mb-2 animate-pulse"></div>
          <div className="h-4 bg-gradient-to-r from-gray-100 to-gray-50 rounded w-96 animate-pulse"></div>
        </div>
        <div className="h-10 bg-gradient-to-r from-gray-200 to-gray-100 rounded-lg w-32 animate-pulse"></div>
      </div>

      {/* Active Users Card Loading */}
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div className="px-6 py-4 border-b border-gray-100">
          <div className="flex items-center">
            <div className="h-5 w-5 bg-gradient-to-r from-gray-200 to-gray-100 rounded-full mr-2 animate-pulse"></div>
            <div className="h-5 bg-gradient-to-r from-gray-200 to-gray-100 rounded w-40 animate-pulse"></div>
          </div>
        </div>
          ...
          <div className="space-y-4">
            {[1, 2, 3].map((i) => (
              <div key={i} className="flex items-center justify-between bg-white border border-gray-200 rounded-xl p-4">
                <div className="flex items-center space-x-4">
                  <div className="h-12 w-12 bg-gradient-to-r from-gray-200 to-gray-100 rounded-full animate-pulse"></div>
                  <div className="space-y-2">
                    <div className="h-4 bg-gradient-to-r from-gray-200 to-gray-100 rounded w-32 animate-pulse"></div>
                    <div className="h-3 bg-gradient-to-r from-gray-100 to-gray-50 rounded w-40 animate-pulse"></div>
                    <div className="flex space-x-2">
                      <div className="h-5 bg-gradient-to-r from-gray-200 to-gray-100 rounded w-16 animate-pulse"></div>
                      <div className="h-5 bg-gradient-to-r from-gray-200 to-gray-100 rounded w-20 animate-pulse"></div>
                    </div>
                  </div>
                </div>
                <div className="space-y-1">
                  <div className="h-3 bg-gradient-to-r from-gray-100 to-gray-50 rounded w-24 animate-pulse"></div>
                  <div className="h-3 bg-gradient-to-r from-gray-100 to-gray-50 rounded w-32 animate-pulse"></div>
                  <div className="flex space-x-2 mt-2">
                    <div className="h-7 bg-gradient-to-r from-gray-200 to-gray-100 rounded w-24 animate-pulse"></div>
                    <div className="h-7 bg-gradient-to-r from-gray-200 to-gray-100 rounded w-16 animate-pulse"></div>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* Pending Invitations Card Loading */}
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div className="px-6 py-4 border-b border-gray-100">
          <div className="flex items-center">
            <div className="h-5 w-5 bg-gradient-to-r from-gray-200 to-gray-100 rounded-full mr-2 animate-pulse"></div>
          ...
          <div className="space-y-4">
            {[1, 2].map((i) => (
              <div key={i} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                <div className="flex items-center space-x-4">
                  <div className="h-10 w-10 bg-gradient-to-r from-gray-200 to-gray-100 rounded-full animate-pulse"></div>
                  <div className="space-y-2">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={() => setIsInviteDialogOpen(true)}
          className="flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-all hover:scale-105 active:scale-95"
        >
          <svg className="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
          </svg>
        ...
        <div className="px-6 py-4">
          {users.length === 0 ? (
            <div className="text-center py-8">
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-4 rounded-full w-20 h-20 mx-auto flex items-center justify-center mb-4">
              ...
              <p className="text-gray-600 mb-6 max-w-md mx-auto">
                Start by inviting users to your organization to manage their dashboard access.
              </p>
              <button
                onClick={() => setIsInviteDialogOpen(true)}
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm transition-all duration-300 hover:shadow-md">
        ...
        <div className="px-6 py-4">
          {pendingNumbers.length === 0 ? (
            <div className="text-center py-8">
              <div className="bg-gradient-to-br from-green-50 to-emerald-50 p-4 rounded-full w-20 h-20 mx-auto flex items-center justify-center mb-4">
            ...
            <div className="space-y-4">
              {pendingNumbers.map((invitation) => (
                <div key={invitation._id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                  <div className="flex items-center space-x-4">
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <form onSubmit={handleSendInvitation} className="space-y-6">
                ...
                {/* Info Alert */}
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-lg p-4 mt-6">
                  <p className="text-sm text-gray-600 leading-relaxed">
                    The user will receive an email with instructions to set up their account and password.
                  </p>
                </div>
                ...
                <div className="flex justify-end space-x-3 pt-6">
                  <button
                    ...
                    disabled={isLoading}
                    className="px-4 py-2.5 border border-gray-300 rounded-lg hover:bg-gray-50 text-gray-700 disabled:opacity-50 transition-colors"
                  >
                    Cancel
                  </button>
                  <button
